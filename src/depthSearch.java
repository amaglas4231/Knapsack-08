...
public static int SkippedSpaces = 2;
public static int BackTrackingAttempts = 0;
public static int MaximumAttempts = 10000000;
...
public static int[] depthSearching(ArrayList<Integer> b){
    for(int z=0 ; z<b.size() ; z++){
        AllPentominoes.get(z).setAmount(b.get(z));
    }
        EmptyContainer();
            ...
            /*    System.out.println("POSITION :");
                ...
                int[] LastTrip = {UserVariables.container.length-1, UserVariables.container[0].length-1,UserVariables.container[0][0].length-1};
                if(Arrays.equals(position, LastTrip) && MaximumAttempts>BackTrackingAttempts){ //We only access here when there are no positions left in container 
                    if(score>Bestscore){
 ...
 //   System.out.println(Bestscore);
  ...
  //  System.out.println("STATISTICS "+"\n T left : "+AllPentominoes.get(0).getAmount()+"\n P left : "+AllPentominoes.get(1).getAmount()+" \n L left : "+AllPentominoes.get(2).getAmount());
   ...
   // System.out.println("Stacksize "+StackBarbuc.size());
    ...
    while(value.hasNext()){
       ...
       // System.out.println("Placed at ["+popped[0][0]+", "+popped[0][1]+", "+popped[0][2]+"]  with piece and rotation  ["+popped[1][0]+" , "+popped[1][1]+" ]");
    }
    System.out.println("Score StackScore : "+stempScore);
    System.out.println(" Number of T "+NbofT+" \n Number of P "+NbofP+" \n Number of L "+NbofL);
    int[] resultGA = {stempScore-(AllPentominoes.get(0).getAmount()*5 + AllPentominoes.get(1).getAmount()*4 + AllPentominoes.get(2).getAmount()*3), ((NbofT+NbofL+NbofP)*5)};
    return resultGA;
