...
public static void main(String[]args) /*throws IOException*/{
 ...
 /*   //Best value filling
  ...
  //Best volume filling
    ...
    /*
    //Code block to test the impact of the attempts of backtracking, and number of skipped spaces
...
for(int y = 1 ; y<6 ; y++){ 
    ...
    for(int j = 1 ; j<1000000 ; j*=10){
        ...
        for(int i = 0 ; i<50 ; i++){
            storageReal = depthSearching(Population.Population.get(i));
...
csvFile.write("-------------------I took "+totalTime+" to run---------------------"+"\n");
}
    }
    csvFile.close();
System.out.println(Arrays.deepToString(UserVariables.container));
...
//The number of each type of Pento is not actually used because we use the numbers generated by the GA
...
//I actually store my Parcels into a Pentominoe object, because they both have the same constructor, so I don't have to do any modifications on my classes 
...
public static int[] depthSearching(ArrayList<Integer> b){
     ...
     //For negative values in Chromosome, penalize the chromosome   
        ...
        EmptyContainer();
            for(int k = 0 ;k< UserVariables.container.length ; k++){
                    ...
                    for(int m = 0 ;m< UserVariables.container[0][0].length ; m++){
                     ...
                     if(UserVariables.container[k][l][m]==0) { //true, as long as there is still space in container
                            for(i = 0 ; i<AllPentominoes.size() ; i++){
                                if(UserVariables.container[k][l][m]!=0){ //will be the case then break and goes to end of loop i
                                    break;
                                }
                                ...
                                for(j = 0 ; j<AllPentominoes.get(i).getRotation().length ; j++){
                                    ...
                                    if(BackTracking){
                                        ...
                                        if(JA<AllPentominoes.get(IA).getRotation().length){
                                    ...
                                    if(UserVariables.container[k][l][m]!=0){ //will be the case, then break and goes to next iteration of i 
                                        break;
                                }
                                
                                //If not possible to put pento at current position, then try n number of time later : varying the parameter m 
                                    for(int n = 0 ; n<SkippedSpaces ; n++){
                                        ...
                                        int[]piecesAndRotations = {i,j};
                                            ...
                                            if(addParcel(AllPentominoes.get(i).getRotation()[j],positionSkip, piecesAndRotations)&& AllPentominoes.get(i).getAmount()>0){ //try to put a pento, if succeed, break and goes to next iteration of j
                                                AllPentominoes.get(i).setAmount(AllPentominoes.get(i).getAmount()-1);
                                                //If false, goes to next iteration of skipped space, then goes to j and try differnt rotation
                                                int[][] a = {{k,l,sum},{i,j}};
                ...
                if(Arrays.equals(position, LastTrip) && MaximumAttempts>BackTrackingAttempts){ //We only access here when there are no positions left in container and we still have backtracking attempst remaining
                    if(score>Bestscore){
                        Bestscore=score;
                    ...
                    BackTracking=true;                   
                    score-=AllPentominoes.get(i).getValue();
                    AllPentominoes.get(i).setAmount(AllPentominoes.get(i).getAmount()+1); //since we just removed a pento, we increase the number available again
   ...
   /* System.out.println("Score StackScore : "+stempScore);
    ...
    System.out.println("Number of Pentoes left : ["+ " T : "+(b.get(0)-NbofT)+" P : "+(b.get(1)-NbofP)+" L : "+(b.get(2)-NbofL)+" ]");
  //  System.out.println("Volume occupied parcels: "+ (1320 -(NbofT*3.375 + NbofP*2.25+NbofL*2)));
    System.out.println("Volume left pentoes: "+ (1320 -((NbofT+NbofL+NbofP)*5)));*/

    //when printing the inside of the container here, there is nothing inside.... saddddd 
    return resultGA;
}
