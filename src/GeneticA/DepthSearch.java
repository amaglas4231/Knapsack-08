...
public class DepthSearch {
    ...
    // We actually store the Parcels into a Pentominoe object, because they both
    // have
    // the same constructor, so we don't have to do any modifications on the classes
        ...
        for (int k = 0; k < UserVariables.container.length; k++) {
            for (int l = 0; l < UserVariables.container[0].length; l++) {
                for (int m = 0; m < UserVariables.container[0][0].length; m++) {
                    ...
                    if (UserVariables.container[k][l][m] == 0) { // true, as long as there is still space in container
                        for (i = 0; i < AllPentominoes.size(); i++) {
                            if (UserVariables.container[k][l][m] != 0) {
                                // will be the case then break and goes to end of loop i
                                break;
                            }
                            ...
                            for (j = 0; j < AllPentominoes.get(i).getRotation().length; j++) {
                                ...
                                if (UserVariables.container[k][l][m] != 0) {
                                    // will be the case, then break and goes to next iteration of i
                                    break;
                                }
                                ...
                                for (int n = 0; n < SkippedSpaces; n++) {
                                    ...
                                    if (addParcel(AllPentominoes.get(i).getRotation()[j], positionSkip,
                                            piecesAndRotations) && AllPentominoes.get(i).getAmount() > 0) {
                                        // try to put a pento, if succeed, break and goes to next iteration of j
                    ...
                    if (Arrays.equals(position, LastTrip) && MaximumAttempts > BackTrackingAttempts) {
                        // We only access here when there are no positions left in container and we
                        // still have backtracking attempst remaining
                        ...
                        // since we just removed a pento, we increase the number available again
                        AllPentominoes.get(i).setAmount(AllPentominoes.get(i).getAmount() + 1);
                    }
                }
            }
